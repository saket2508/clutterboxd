...
router.post('/add/movie', authorize, async(req, res) => {
    try{
        ...
        res.json({"newItem": newItem.rows[0], success: true, message: `Added ${item.title} to list`})
    } catch(err){
        console.error(err.message)
...
router.post('/add/tv', authorize, async(req, res) => {
    try{
        ...
        res.json({"newItem": newItem.rows[0], success: true, message: `Added ${newItem.rows[0].title.toUpperCase()} to list`})
    } catch(err){
        console.error(err.message)
        res.status(500).json("Could not add item to list")
    }
...
router.delete('/delete/:media/:id', authorize, async(req, res) => {
    try{
        ...
        res.json({success: true, message: `Removed ${deletedItem.rows[0].title.toUpperCase()} from list`})
    } catch(err){
        console.error(err.message)
